...
/* Phase 3 receiver: poll for the LAST sentinel (which forces all 7
 * deferred WQEs to land first since they share a single SRD QP), then
 * verify each of the BURST sentinels is in place at its respective
 * offset. */
__global__ void wait_burst_kernel(volatile int *dst_base, uint64_t timeout_cycles, int *result_out)
{
  if (threadIdx.x == 0 && blockIdx.x == 0)
    printf("RECEIVER[burst]: polling dst[%d]=%p for value 0x%x\n",
           BURST - 1, (void*)&dst_base[BURST - 1],
           (unsigned)(VAL_BURST_BASE + BURST - 1));

  /* Step 1: wait for the LAST sentinel (post BURST-1, default-flag, doorbell). */
  uint64_t start = clock64();
  int last_expected = (int)(VAL_BURST_BASE + BURST - 1);
  int seen_last;
  uint64_t iters = 0;
  do {
    seen_last = dst_base[BURST - 1];
    iters++;
    if (clock64() - start > timeout_cycles) break;
  } while (seen_last != last_expected);

  if (threadIdx.x == 0 && blockIdx.x == 0)
    printf("RECEIVER[burst]: dst[%d]=0x%x after %lu iters %s\n",
           BURST - 1, (unsigned)seen_last, (unsigned long)iters,
           seen_last == last_expected ? "(last seen)" : "(TIMEOUT)");

  /* Step 2: verify all BURST sentinels are in place. */
  __threadfence_system();
  int errors = 0;
  for (int i = 0; i < BURST; i++) {
    int got = dst_base[i];
...
int main(int argc, char **argv)
{
  ...
  /* Multi-rail variant: run each phase twice, once with ctxId=0 and
   * once with ctxId=1. Plugin's Policy A maps rail = contextId % num_rails;
   * with num_rails=2 on p5en, ctxId=0 -> rail 0 and ctxId=1 -> rail 1.
   * Both must succeed for the multi-rail path to be considered functional. */
  for (int ctxId = 0; ctxId < 2; ctxId++) {
    if (rank == 0) printf("\n=== Round ctxId=%d (rail=%d) ===\n", ctxId, ctxId);
    int v_nosig = VAL_NO_SIGNAL + ctxId;
    int v_sig   = VAL_WITH_SIGNAL + ctxId;

    /* Reset dst before each round on rank 1 so we can cleanly observe
     * a fresh write per (ctxId, phase). */
    if (rank == 1) {
      CUDACHECK(cudaMemset(dst_gpu, 0, BUF_SIZE));
    }
    ...
    if (rank == 0) {
      putvalue_no_signal_kernel<<<1, 1, 0, stream>>>(
          *devCommPtr, /*ctxId=*/ctxId,
          dstWin, /*dstOff=*/0,
          /*peer=*/1, /*value=*/v_nosig);
      CUDACHECK(cudaStreamSynchronize(stream));
      printf("R0[ctx=%d]: phase1 (no-signal) sender done\n", ctxId);
    } else {
      wait_no_signal_kernel<<<1, 1, 0, stream>>>(dst_gpu, v_nosig, WAIT_TIMEOUT_CYCLES);
      CUDACHECK(cudaStreamSynchronize(stream));
      int seen;
      CUDACHECK(cudaMemcpy(&seen, dst_gpu, sizeof(int), cudaMemcpyDeviceToHost));
      printf("R1[ctx=%d]: phase1 (no-signal) final dst=0x%x %s\n",
             ctxId, (unsigned)seen, seen == v_nosig ? "PASS" : "FAIL");
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      putvalue_with_signal_kernel<<<1, 1, 0, stream>>>(
          *devCommPtr, /*ctxId=*/ctxId,
          ...
          /*signalId=*/0);
      CUDACHECK(cudaStreamSynchronize(stream));
      printf("R0[ctx=%d]: phase2 (signal) sender done\n", ctxId);
    } else {
      wait_signal_kernel<<<1, 1, 0, stream>>>(
          *devCommPtr, /*ctxId=*/ctxId,
          dst_gpu, v_sig, /*signalId=*/0);
      CUDACHECK(cudaStreamSynchronize(stream));
      int seen;
      CUDACHECK(cudaMemcpy(&seen, dst_gpu, sizeof(int), cudaMemcpyDeviceToHost));
      printf("R1[ctx=%d]: phase2 (signal) final dst=0x%x %s\n",
             ctxId, (unsigned)seen, seen == v_sig ? "PASS" : "FAIL");
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      putvalue_burst_aggregate_kernel<<<1, 1, 0, stream>>>(
          *devCommPtr, /*ctxId=*/ctxId,
          ...
          /*peer=*/1);
      CUDACHECK(cudaStreamSynchronize(stream));
      printf("R0[ctx=%d]: phase3 (aggregate-requests) sender done\n", ctxId);
    } else {
      ...
      printf("R1[ctx=%d]: phase3 (aggregate-requests) errors=%d  values=[", ctxId, errors_host);
      for (int i = 0; i < BURST; i++) {
